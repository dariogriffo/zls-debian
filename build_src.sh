...
if [ -z "$ZLS_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <zls_version> <build_version>"
    echo "Example: $0 0.14.0 1"
    exit 1
fi

PACKAGE_NAME="zls"
ORIG_TARBALL="${PACKAGE_NAME}_${ZLS_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${ZLS_VERSION}"

echo "Creating Debian/Ubuntu source packages for zls ${ZLS_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/zigtools/zls/archive/refs/tags/${ZLS_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${ZLS_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as zls-0.x.y/ which matches our BUILD_DIR
...
zls (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${ZLS_VERSION}.
